...
** Headlines
outline 第一个功能就是 Headlines， 也就是 “*” "**" "*****" 这样的标题分级。 在youdao markdown
中使用的是"#"
** Visibility Cycling
这就是说，每一节的可见性是一个圈，通过那行首，按Tab， 可以切换可视性。
以下是快捷键和函数。
TAB (org-cycle)
...
* Table
等我有时间了看，现在最要紧的是解决 有道云笔记没法插图的问题
* Hyperlinks
...
** Paragraphs
BEGIN_VERSE END_VERSE  verse的意思是诗文， 在这个块下，你怎么写的它就怎么显示.
#+BEGIN_VERSE
...
BEGIN_QUOTE END_QUOTE 引言引用 ， 一般在引用名人名言时使用。 它会将---以前的都放在一行。
#+BEGIN_QUOTE
 Great clouds overhead
 Tiny black birds rise and fall
 Snow covers Emacs
...
BEGIN_CENTER END_CENTER 居中引用，在块里使用\\是强制换行
#+BEGIN_CENTER
Everything should be made as simple as possible, \\
but not any simpler
#+END_CENTER

BEGIN_EXAMPLE 也是比较常用的。
...
* ref
1. org manual  https://orgmode.org/manual/index.htm
